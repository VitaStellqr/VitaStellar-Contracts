...
impl PharmaSupplyChainContract {
    ...
    pub fn create_batch(
        ...
        env.events().publish(
            (String::from_str(&env, "vst/pharma_supply"), Symbol::new(&env, "batch_create")),
    ...
    pub fn create_shipment(
        ...
        env.events()
            .publish((String::from_str(&env, "vst/pharma_supply"), Symbol::new(&env, "ship")), (shipment_id, batch_id));
...
.Groups[1].Value
    if ($imports -notmatch 'String') { $imports += ', String' }
    if ($imports -notmatch '\bSymbol\b') { $imports += ', Symbol' }
    "use soroban_sdk:{$imports};"
